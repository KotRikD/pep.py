...
def sendMessage(fro = "", to = "", message = "", token = None, toIRC = True):
	...
	try:
		...
		if isChannel:
			...
			# Make sure the recipient is not restricted or we are Charlotte
			if recipientToken.restricted and fro != glob.BOT_NAME:
				raise exceptions.userRestrictedException()
		...
		# Spam protection (ignore Charlotte)
		...
		if not (message.startswith("\x01ACTION is playing") and to.startswith("#spect_")):
			...
			else: # Spaghetti code. I'm not sure how any() works because I'm fucking retarded. :)
				"""
				checks = ['cheat', 'aqn', 'aqui', 'hq', 'cmy', 'qosu', 'fuck akat']
				if any(x in message for x in checks):
					log.cmyui("[PRIVATE - DETECTED] {fro} @ {to}: {message}".format(fro=token.username, to=to, message=message.encode("latin-1").decode("utf-8"), discord='cm'))
					glob.schiavo.sendCM("**{fro} @ {to}:** {message}".format(fro=token.username, to=to, message=message.encode("latin-1").decode("utf-8")))
				else:
				"""
